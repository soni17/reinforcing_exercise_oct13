...
varb[:response][:results].each do |element|
   element[:views] = 0
end

def read_article(arg)
  article = arg[:response][:results].sample
  article[:views] += 1
  return article
end

def display_views(arg)
  arg[:response][:results].each do |element|
     puts "Title: #{element[:webTitle]}"
     puts "View count: #{element[:views]}"
     puts " "
  end
end


25.times { read_article(varb) }
